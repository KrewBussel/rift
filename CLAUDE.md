...
**Firms whose opportunities are not all rollovers** (the common case) use a dedicated Wealthbox **opportunity pipeline** (e.g. "Rollover") so only rollover opps ever reach the mapped bookend stages. This works because Wealthbox stage ids are pipeline-scoped — each pipeline has its own "Proposal Accepted"/"Won" with distinct ids, and the inbound poll filters by exact stage id. Support in code: stage rows carry `pipeline` (id) from `/categories/opportunity_pipelines` + `/categories/opportunity_stages`; `crmClient.getStages()` annotates each stage with `pipelineId`/`pipelineName`; stage pickers prefix options with the pipeline name whenever stages span multiple pipelines (`src/components/crmStageOptions.ts`).

**Choosing the source pipeline.** `CrmConnection.pipelineId` / `pipelineName` record which pipeline feeds Rift. Set it in **Settings → Integrations → Stage sync**; the stage pickers then only offer that pipeline's stages, and a selection outside it is cleared rather than left invisibly mapped. `suggestBookendStages(stages, selectedPipelineId?)` resolves its search pool in priority order: the explicitly selected pipeline (never looking outside it, even with no match) → a pipeline named like "rollover"/"rift" → all stages. The connect endpoint auto-selects the pipeline when the account has exactly one, and never overwrites an existing choice on token rotation.

**Not every plan can create pipelines.** The number of opportunity pipelines a firm can create depends on their Wealthbox plan tier — capped accounts have only "Default", which necessarily carries non-rollover business too. For those firms, `CrmConnection.requireRolloverFields` (opt-in, default false, exposed as the "Rollovers only" toggle) makes inbound creation conditional on the opportunity carrying all three Rift custom fields. Presence is the rollover signal; *validity* is separate — an Account Type that's set but unmappable still creates a case flagged `needsReview`, because it's plainly a rollover with a bad value. Filtered opportunities are counted in `PollResult.filtered` and surfaced in the "Sync Wealthbox" confirmation, so a filter that's rejecting everything never looks like an idle sync.
